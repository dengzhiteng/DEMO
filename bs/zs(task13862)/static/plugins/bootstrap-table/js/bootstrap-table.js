...
!function ($) {
    ...
    var getScrollBarWidth = function () {
        if (cachedWidth === null) {
            ...
            if($(".fixed-table-scroll-outer").size() == 0){
                var inner = $('<p/>').addClass('fixed-table-scroll-inner'),
                outer = $('<div/>').addClass('fixed-table-scroll-outer');
                outer.append(inner);
                $('body').append(outer);
            }else{
                var inner = $('.fixed-table-scroll-inner'), outer = $('.fixed-table-scroll-outer');
            }
    ...
    BootstrapTable.LOCALES['zh-CN'] = BootstrapTable.LOCALES['cn'] = {
        ...
        formatDetailPagination: function (totalRows) {
            return sprintf('总共 %s 条记录', totalRows);
        },
    ...
    BootstrapTable.prototype.initTable = function () {
        ...
        if (!this.$header.length) {
            this.$header = $('<thead/>').appendTo(this.$el);
        }
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, columns) {
            ...
            $.each(columns, function (j, column) {
                ...
                html.push(sprintf('<div class="th-inner %s"><div class="th-text">', that.options.sortable && column.sortable ?
                    'sortable both' : ''));
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            var pageNumber = [
                    sprintf('<span class="btn-group %s">',
                        this.options.paginationVAlign === 'top' || this.options.paginationVAlign === 'both' ?
                            'dropdown' : 'dropup'),
                    '<button type="button" class="btn btn-default ' +
                    sprintf(' btn-%s', this.options.pageListBtnSize) +
                    ' dropdown-toggle" data-toggle="dropdown">',
            ...
            if(this.options.usePageGo){
                var selectPageHtml = '<div class="select-page">'+
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            $last = this.$pagination.find('.page-last');
            $pageGoBtn = this.$pagination.find('.select-page .btn');
            ...
            $pageGoBtn.off('click').on('click', $.proxy(this.onPageGo, this));
        }
    };
    ...
    BootstrapTable.prototype.onPageGo = function (event) {
        var ipt = $(event.currentTarget).siblings().find(":text"),
            userPages = $.trim(ipt.val());
        if(isNaN(userPages) || userPages == '' || userPages == 0){ return }

        if (this.options.pageNumber === userPages || userPages > this.options.totalPages) {
            return;
        }
        this.options.pageNumber = userPages;
        this.updatePagination(event);
    };
    ...
    //动态分配列宽高
    BootstrapTable.prototype.setArea = function(){
        var that = this,
            options = that.config
            ,colNums = 0 //列个数
    ...
    BootstrapTable.prototype.fitHeader = function () {
        ...
        scrollWidth = fixedBody.offsetWidth > fixedBody.clientWidth ? getScrollBarWidth() : 0;

        //scrollHeight = fixedBody.offsetHeight > fixedBody.clientHeight + this.$header.outerHeight() ? getScrollBarWidth() : 0;
        ...
        this.$body.find('>tr:first-child:not(.no-records-found) > *').each(function (i) {
            ...
            if (that.options.detailView && !that.options.cardView) {
                if (i === 0) {
                    that.$header_.find('th.detail').find('.fht-cell').width($this.outerWidth() - 1);
                }
                index = i - 1;
            }
            
            that.$header_.find(sprintf('th[data-field="%s"]', visibleFields[index])).find('.fht-cell').width($this.outerWidth() - 1);
        });
    ...
    BootstrapTable.prototype.fitFooter = function () {
        ...
        this.$body.find('>tr:first-child:not(.no-records-found) > *').each(function (i) {
            ...
            $footerTd.eq(i).find('.fht-cell').width($this.outerWidth() - 1);
        });
    };
